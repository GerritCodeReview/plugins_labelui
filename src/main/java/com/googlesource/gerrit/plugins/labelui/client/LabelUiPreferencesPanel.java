...
public class LabelUiPreferencesPanel extends VerticalPanel {
  private final static String HIDE_LABELS_CLASS = Plugin.get().getName() + "-hideLabels";
      ...
      public void onChange(ChangeEvent event) {
        ...
        new RestApi("accounts").id("self")
            ...
            .put(info, new AsyncCallback<PreferencesInfo>() {
              @Override
              public void onSuccess(PreferencesInfo result) {
                RootPanel.getBodyElement().removeClassName(HIDE_LABELS_CLASS);
                if (result.ui() != LabelUi.DEFAULT) {
                  RootPanel.getBodyElement().addClassName(HIDE_LABELS_CLASS);
                }
              }
